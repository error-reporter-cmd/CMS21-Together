...
public static class UIMenu
{
	...
	/// <summary>
	/// Sets up the main menu with the multiplayer button integration
	/// </summary>
	public static void SetupMainMenu()
	{
		// Reposition the default Play button to make room for multiplayer button
		...
		// Load custom texture for multiplayer button
		try
		{
		...
		{
			MelonLoader.MelonLogger.Warning($"Failed to load multiplayer button texture: {ex.Message}");
		}
	}

	/// <summary>
	/// Sets up the multiplayer menu with host/join options
	/// </summary>
	public static void SetupMultiplayerMenu()
	{
		...
		var joinBtn = UIElements.CreateButton(UICore.MP_Main.transform,
			"Join a game", () => { UICore.ShowPanel(UICore.MP_Lobby.gameObject); });
	...
	/// <summary>
	/// Sets up the host menu with save file selection
	/// </summary>
	public static void SetupHostMenu()
	{
		...
		while (i < 4)
		{
			var saveBtn = UIElements.CreateButton(UICore.MP_Host.transform, $"Save Slot {i + 1}", null);
